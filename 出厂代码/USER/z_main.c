...
void loop_ps2_car(void) {
	static int run_speed, car_turn, run_speed_bak, car_turn_bak;
	
	if(psx_buf[1] != PS2_LED_RED)return;
	
	if(abs_int(127 - psx_buf[6]) < 5 )psx_buf[6] = 127;
	if(abs_int(127 - psx_buf[7]) < 5 )psx_buf[7] = 127;
	
	//总线马达设置	
	run_speed = (127 - psx_buf[6]) * 8;
	car_turn = (127 - psx_buf[7]) * 5;
	
	if(abs_int(run_speed) < 100)run_speed = 0;
	if(run_speed > 1000)run_speed = 1000;
	if(run_speed < -1000)run_speed = -1000;
	
	if(abs_int(car_turn) < 100)car_turn = 0;
	if(car_turn > 800)car_turn = 800;
	if(car_turn < -800)car_turn = -800;
	
	
	
	if(run_speed != run_speed_bak || car_turn != car_turn_bak) {
		
		//uart1_send_str((u8*)"ps2:");
		car_run(run_speed, run_speed, run_speed, run_speed,car_turn);
		run_speed_bak = run_speed;
		car_turn_bak = car_turn;
	}
}
...
返回值：  无  
*************************************************************/
void car_run(int speedlq, int speedrq, int speedlh, int speedrh,int speedt) {	
	sprintf((char *)cmd_return, "{#006P%04dT0000!#007P%04dT0000!#008P%04dT0000!#009P%04dT0000!#010P%04dT1000!}", (int)(1500+speedlq), (int)(1500-speedrq), (int)(1500+speedlh), (int)(1500-speedrh),(int)(1500-speedt));
