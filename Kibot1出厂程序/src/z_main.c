...
u8 psx_buf[9] = {0x00, 0x00, 0x00, 0x00, 0x00,
                 0x00, 0x00, 0x00, 0x00};  // 存储手柄的数据
...
////add
/*
 * 06 ->右后电机 07->左后电机 10->舵机
 * 电机控制: <1500 前进 =1500 停机 >1500 后退
 * 舵机控制: <1500 左转 =1500 复位 >1500 右转
 */

const char *sAction[7];
const int msDelay[] = {10000, 1000, 500, 2000, 5000, 1000, 5000};
/////

int main(void) {
    /////
    u32 startTime = millis();  // millis()传递当前时间(单位为ms)
    int i = 0;                 // 初始化数组下标
    /////
    setup_rcc();        // 初始化时钟
    ...
    sAction[0] = actionCmdGen(SERVO, "1250", NULL);
    sAction[1] = actionCmdGen(SERVO, "1750", NULL);
    sAction[4] = actionCmdGen(SERVO, "1000", NULL);
    sAction[5] = actionCmdGen(SERVO, "2000", NULL);
    sAction[6] = actionCmdGen(SERVO, "1500", NULL);
    while (1) {
        loop_nled();
        while (millis() - startTime < msDelay[i])
            ;
        tb_usart3_send_str((u8 *)sAction[i++]);
        startTime = millis();
        if (i >= 7) i = 0;
    }
}
...
void setup_w25q64(void) {  // 初始化存储器W25Q64
    ...
    w25x_read((u8 *)(&eeprom_info), W25Q64_INFO_ADDR_SAVE_STR,
              sizeof(eeprom_info));  // 读取全局变量
...
void loop_action(void) {
    // 通过判断舵机是否全部执行完毕
    // 并且是执行动作组group_do_ok尚未结束的情况下进入处理
...
void car_set(int car_left, int car_right) {
    ...
    sprintf((char *)cmd_return, "{#006P%04dT0000!#007P%04dT0000!}",
        ...
        for (i = 0; i < 16; i++) {  // 16个按键一次轮询
            if ((1 << i) & temp) {
                ...
                if ((1 << i) & bak) {  // press 表示按键按下了
                    ...
                    if (mode == PS2_LED_RED) {
                        memcpy((char *)uart_receive_buf,
                               (char *)pre_cmd_set_red[i],
                               strlen(pre_cmd_set_red[i]));
                    } else if (mode == PS2_LED_GRN) {
                        memcpy((char *)uart_receive_buf,
                               (char *)pre_cmd_set_grn[i],
                               strlen(pre_cmd_set_grn[i]));
                    } else
                        continue;
                    ...
                    if (str_contain_str(uart_receive_buf, (u8 *)"$")) {
                        uart1_close();
                        uart1_get_ok = 0;
                        strcpy((char *)cmd_return,
                               (char *)uart_receive_buf + 11);
                    ...
                    if (pos) {
                        if (str_contain_str(uart_receive_buf + pos,
                            ...
                            // uart1_get_ok = 0;
                            strcpy((char *)cmd_return,
                                   (char *)uart_receive_buf + pos);
                            cmd_return[strlen((char *)cmd_return) - 1] = '\0';
                            strcpy((char *)uart_receive_buf,
                                   (char *)cmd_return);
                            parse_cmd(uart_receive_buf);
                            // uart1_get_ok = 1;
                            // uart1_mode = 1;
...
/*
    ...
    $KMS:x,y,z,time! //坐标单位mm，时间单位ms 测试点(0,120,30,1000) ->
   $KMS:0,120,20,2000!
*/
// 命令解析函数
void parse_cmd(u8 *cmd) {
    ...
    if (pos = str_contain_str(cmd, (u8 *)"$DRS!"), pos) {
        ...
        if (sscanf((char *)cmd, "$DGT:%d-%d,%d!", &group_num_start,
                   &group_num_end, &group_num_times)) {
            group_do_ok = 1;
            if (group_num_start != group_num_end) {
                do_start_index = group_num_start;
        ...
        for (i = 0; i < DJ_NUM; i++) {
            ...
            duoji_doing[i].inc = (duoji_doing[i].aim - duoji_doing[i].cur) /
                                 (duoji_doing[i].time / 20.000);
        }
        ...
        if (sscanf((char *)cmd, "$KMS:%d,%d,%d,%d!", &int1, &int2, &int3,
                   &int4)) {
...
void save_action(u8 *str) {
    ...
    if (str[1] == '$' && str[2] == '!') {
        ...
        if (sscanf((char *)str, "<$DGT:%d-%d,%d!>", &group_num_start,
                   &group_num_end, &group_num_times)) {
            if (group_num_start == group_num_end) {
                w25x_read(eeprom_info.pre_cmd, group_num_start * ACTION_SIZE,
                          ACTION_SIZE);
            } else {
...
int getMaxTime(u8 *str) {
    ...
    while (str[i]) {
        if (str[i] == 'T') {
            tmp_time = (str[i + 1] - '0') * 1000 + (str[i + 2] - '0') * 100 +
...
void parse_action(u8 *uart_receive_buf) {
    ...
    if (uart_receive_buf[0] == '#' && uart_receive_buf[4] == 'P' &&
        uart_receive_buf[5] == 'S' && uart_receive_buf[6] == 'C' &&
        uart_receive_buf[7] == 'K' && uart_receive_buf[12] == '!') {
        index = (uart_receive_buf[1] - '0') * 100 +
                (uart_receive_buf[2] - '0') * 10 + (uart_receive_buf[3] - '0');
        bias = (uart_receive_buf[9] - '0') * 100 +
               (uart_receive_buf[10] - '0') * 10 + (uart_receive_buf[11] - '0');
        if ((bias >= -500) && (bias <= 500) && (index < DJ_NUM)) {
            if (uart_receive_buf[8] == '+') {
            } else if (uart_receive_buf[8] == '-') {
                bias = -bias;
            }
            aim_temp = duoji_doing[index].cur + 0.043198 -
        ...
        return;
               ...
               uart_receive_buf[7] == 'T' && uart_receive_buf[8] == '!') {
        index = (uart_receive_buf[1] - '0') * 100 +
                (uart_receive_buf[2] - '0') * 10 + (uart_receive_buf[3] - '0');
        if (index < DJ_NUM) {
            duoji_doing[index].inc = 0;
    ...
    while (uart_receive_buf[i]) {
        if (uart_receive_buf[i] == '#') {
            ...
            if (index < DJ_NUM && (pwm <= 2500) && (pwm >= 500) &&
                (time <= 10000)) {
                // duoji_doing[index].inc = 0;
                ...
                // time, duoji_doing[index].inc); uart1_send_str(cmd_return);
            }
...
void rewrite_eeprom(void) {
    ...
    w25x_write((u8 *)(&eeprom_info), W25Q64_INFO_ADDR_SAVE_STR,
               sizeof(eeprom_info));
    spiFlahsOn(0);
}
...
/*
 *device: RL_MOTOR,L_MOTOR,R_MOTOR, SERVO
 */
char *actionCmdGen(int device, char *intensity, char *intensity2) {
    char *buf = malloc(sizeof(char) * 30);
    if (intensity2 != NULL) {
        switch (device) {
            case RL_MOTOR:
                sprintf(buf, "{#006P%sT0000!#007P%sT0000!}", intensity,
                        intensity2);
                return buf;
            case SERVO:
                sprintf(buf, "{#010P%sT0000!}", intensity);
                return buf;
        }
    } else {
        switch (device) {
            case L_MOTOR:
                sprintf(buf, "{#006P1500T0000!#007P%sT0000!}", intensity);
                return buf;
            case R_MOTOR:
                sprintf(buf, "{#006P%sT0000!#007P1500T0000!}", intensity);
                return buf;
            case SERVO:
                sprintf(buf, "{#010P%sT0000!}", intensity);
                return buf;
